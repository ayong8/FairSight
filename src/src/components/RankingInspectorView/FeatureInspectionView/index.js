...
class FeatureInspectionView extends Component {
  ...
  renderCategoricalFeatureForOutlier(feature) {
    ...
    const featureBinForInstances = d3
      .histogram()
      .domain([0, 1])
      .thresholds([0, 1])
      .value(d => d.features[name])(instances);

    const xFeatureScaleForOutliers = d3
        .scaleBand()
        .domain(featureBinForInstances.map(d => d.x0))
        .range([0, this.layout.outlier.feature.widthForInstances * 2]),
      xFeatureScaleForWholeInstances = d3
        .scaleBand()
        .domain(featureBinForInstances.map(d => d.x0))
      ...
      yFrequencyScale = d3
        ...
        .range([
          _self.layout.outlier.feature.svg.height -
            _self.layout.outlier.feature.marginBottom -
            10,
          10 + _self.layout.outlier.feature.marginBottom
        ]);

    const xAxisForInstances = d3
      .select(svgFeature)
      .append('g')
      ...
      .attr(
        'transform',
        'translate(0,' +
          (_self.layout.outlier.feature.heightForInstances -
            _self.layout.outlier.feature.marginBottom +
      ...
      .call(
        d3
          .axisBottom(xFeatureScaleForOutliers)
          .tickSize(0)
          .tickFormat(d => (d === 0 ? 'No' : 'Yes'))
      );
    const r = 3;

    const featureHistogramForOutliers = d3
      .select(svgFeature)
      ...
      .attr('transform', function(d) {
        return (
          'translate(' +
          (_self.layout.outlier.feature.margin +
            (_self.layout.outlier.feature.widthForInstances * 2) / 8) +
          ',' +
      ...
      .each(function(d, i) {
        ...
        d3.select(this)
          ...
          .attr(
            'cx',
            (e, i) =>
              xFeatureScaleForOutliers(featureValue) -
              r +
              Math.floor(i / 15) * 2 * r
          )
    ...
    return {
      outlierDiv: (
        <div className={styles.featureForOutlier}>{svgFeature.toReact()}</div>
      ),
      corrBtnOutliersAndWholeInstances:
        Math.round(corrBtnOutliersAndWholeInstances[name] * 100) / 100
    };
  }
  ...
  renderContinuousFeatureForOutlier(feature) {
    ...
    const instancesBin = d3
      .histogram() // For selected instances
      .domain([min, max])
    ...
    const xFeatureScale = d3
        ...
        .range([
          this.layout.outlier.feature.histoForCont.margin,
          this.layout.outlier.feature.histoForCont.width -
            this.layout.outlier.feature.histoForCont.margin
        ]),
      xAxis = d3
        .select(svgFeature)
        ...
        .call(
          d3
            ...
            .tickValues(
              xFeatureScale.domain().filter(function(d, i) {
                return !(i % 5);
              })
            )
            .tickSizeOuter(0)
    ...
    const featureHistogram = d3
      .select(svgFeature)
      .selectAll('.g_outlier_histogram')
      .data(instancesBin)
      .enter()
      .append('g')
      ...
      .attr('transform', function(d) {
        return (
          ...
          xFeatureScale(d.x0) +
          ',' +
          (_self.layout.outlier.feature.height -
            _self.layout.outlier.feature.marginBottom +
            _self.layout.outlier.feature.margin) +
          ')'
        );
  ...
  renderOrdinalFeatureForOutlier(feature) {
    ...
    const xFeatureScaleForInstances = d3
        .scaleBand()
        .domain(range)
        .range([0, this.layout.outlier.feature.widthForInstances * 2]),
      xFeatureScaleForOutliers = d3
        .scaleBand()
        .domain(range)
    ...
    const xAxisForInstances = d3
      ...
      .call(
        d3
          .axisBottom(xFeatureScaleForInstances)
          .tickValues([0, ...xFeatureScaleForInstances.domain()])
          .tickSizeOuter(0)
      );
    const rScale = d3
      .scaleThreshold()
    ...
    const gFeatureHistogramForInstances = d3
      .select(svgFeature)
      ...
      .attr('transform', function(d) {
        return (
          'translate(' +
          _self.layout.outlier.feature.width / featureValuesCount.length / 4 +
          ',' +
          (_self.layout.outlier.feature.height -
            _self.layout.outlier.feature.marginBottom +
            _self.layout.outlier.feature.margin -
      ...
      .each(function(d, i) {
        ...
        sortedInstances.forEach((e, i) => {
          d3.select(this)
            ...
            .attr(
              'cx',
              xFeatureScaleForInstances(value) + Math.floor(i / 15) * 2 * r
            )
            .attr('cy', -((i % 15) * 2 * r))
            .attr('r', r)
  ...
  renderDistortionOutlierOverview() {
    ...
    const mean =
        sumDistortions.reduce((acc, curr) => acc + curr) /
        sumDistortions.length,
      variance =
        sumDistortions
    ...
    const dataBin = d3
      ...
      .value(d => d.sumDistortion)(instances);

    const xDistortionScale = d3
        .scaleBand()
        .domain(dataBin.map(d => d.x0))
        .range([
          this.layout.outlier.histogram.margin,
          this.layout.outlier.histogram.width -
            this.layout.outlier.histogram.margin
        ]),
      xAxis = d3
        .select(_self.svgOutlier)
        .append('g')
        .attr('class', 'g_x_distortion_axis')
        .attr(
          'transform',
          'translate(0,' +
            (_self.layout.outlier.histogram.height -
              _self.layout.outlier.histogram.marginBottom +
              _self.layout.outlier.histogram.margin) +
            ')'
        )
        .call(
          d3
            .axisBottom(xDistortionScale)
            .tickSizeOuter(0)
            .tickValues(
              xDistortionScale.domain().filter(function(d, i) {
                return !(i % 6);
              })
            )
        ),
        ...
        d3.select(this)
          ...
          .attr(
            ...
            (e, i) =>
              ...
              _self.layout.outlier.histogram.marginBottom -
              r -
              (i * xDistortionScale.bandwidth() + 0.5)
          )
          .attr('r', r)
          .style('fill', e =>
            e.isOutlier ? gs.outlierColor : gs.individualColor
  ...
  renderOriginalRankingForPerturbation() {
    ...
    svgOriginalRanking.setAttribute(
      'height',
      _self.layout.cf.feature.svg.height
    );
    ...
    const topkRankingScale = d3
        .scaleBand()
        .domain(d3.range(1, topk + 1))
        .range([0, plotWidth]),
      nonTopkRankingScale = d3
        .scaleBand()
        .domain(d3.range(topk + 1, n + 1))
        .range([0, plotWidth]),
      previousRankingScale = d3
        .scaleLinear()
        .domain([1, n + 1])
        .range([0.5, rectHeight]);

    const xTopkAxis = d3
        .select(svgOriginalRanking)
        .append('g')
        .attr('class', 'g_x_permutation_axis')
        .attr(
          'transform',
          'translate(' +
            _self.layout.cf.feature.plot.margin +
            ',' +
            (_self.layout.cf.feature.plot.marginTop + rectHeight + 10) +
            ')'
        )
        .call(
          d3
            .axisBottom(topkRankingScale)
            .tickValues([1, ...d3.range(5, topk, 5)])
            .tickSizeOuter(0)
        ),
      xNonTopkAxis = d3
        .select(svgOriginalRanking)
        ...
        .attr(
          'transform',
          'translate(' +
            (_self.layout.cf.feature.plot.margin + plotWidth) +
            ',' +
            (_self.layout.cf.feature.plot.marginTop + rectHeight + 10) +
            ')'
        )
        .call(
          d3
            .axisBottom(nonTopkRankingScale)
            .tickValues([topk + 1, ...d3.range(topk + 10, n, 30), n])
            .tickSizeOuter(0)
        );

    const gTopkRanking = d3
        .select(svgOriginalRanking)
        .append('g')
        ...
        .attr(
          ...
          'translate(' +
            _self.layout.cf.feature.plot.margin +
            ',' +
            _self.layout.cf.feature.plot.marginTop +
            ')'
        ),
      gNonTopkRanking = d3
        .select(svgOriginalRanking)
  ...
  renderFeaturePerturbation(perturbedRankingInstance) {
    ...
    gTopkRanking
      .selectAll('.rect_topk_cf_group_pattern')
      .data(topkInstances)
      .enter()
      .append('rect')
      .attr(
        'class',
        d =>
          'rect_topk_cf_group_pattern rect_topk_cf_group_pattern_' + d.ranking
      )
      .attr('x', d => topkRankingScale(d.ranking))
      .attr('y', d => 20 + 2)
      .attr('width', rectWidth)
    ...
    console.log(
      'statPerturbation',
      statForPerturbation.precisionK,
      stat.precisionK
    );
  ...
  renderFeatureInspectorTable() {
    ...
    const featureInspectorColumns = [
      ...
      {
        title: 'Measure',
        dataIndex: 'corrBtnOutliersAndWholeInstances',
        width: '6%'
      },
    ...
    perturbationResults.forEach(perturbationResult => {
      ...
      const {
        ...
        diffPrecisionK,
        diffSp
      } = _self.renderFeaturePerturbation(perturbationResult);
  ...
  render() {
    if (
      ...
      (!this.props.perturbationResults ||
        this.props.perturbationResults.length === 0) ||
    ...
    return (
      <div
        className={
          this.props.isModelRunning
            ? styles.FeatureInspectionView + ' ' + index.isModelRunning
            : styles.FeatureInspectionView
        }
      >
        <div className={index.title + ' ' + styles.featureInspectorTitle}>
          Feature Inspector
        </div>
