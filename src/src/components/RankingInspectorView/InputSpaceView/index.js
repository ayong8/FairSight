...
class InputSpaceView extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    const inputCoordsPropsChange =
      this.props.inputCoords !== nextProps.inputCoords;
    const selectedInstancePropsChange =
      this.props.selectedInstance !== nextProps.selectedInstance;
    const selectedInstancesPropsChange =
      this.props.selectedInstances !== nextProps.selectedInstances;

    return (
      dataPropsChange ||
      topkPropsChange ||
      inputCoordsPropsChange ||
      selectedInstancePropsChange ||
  ...
  render() {
    ...
    const wholeInstances = _.toArray(inputCoords),
      topkIdx = data.instances
        .map(d => d.idx)
        .slice(0, selectedRankingInterval.to),
      instances = wholeInstances
        .filter(d => topkIdx.indexOf(d.idx) > -1)
        .slice(0, selectedRankingInterval.to);
    ...
    const circles = gCircles
      ...
      .style('fill', d => {
        if (mode === 'GF') {
          let group = d.group;
          return group === 0 ? gs.groupColor1 : gs.groupColor2;
        } else if (mode === 'IF') {
    ...
    return (
      <div className={styles.InputSpaceView}>
        ...
        <div className={styles.IndividualPlotStatusView}>{svg.toReact()}</div>
