...
class OutputSpaceView extends Component {
  ...
  componentWillUpdate(nextProps, nextState) {
    if (this.props.seletedInstance !== nextProps.selectedInstance) {
      if (Object.keys(nextProps.selectedInstance).length !== 0) {
        d3.selectAll('.rect_output.selected')
          .style('stroke', d3.rgb(gs.groupColor1).darker())
          .style('stroke-width', 0.5)
          .classed('selected', false);
        d3.selectAll('.rect_output_' + nextProps.selectedInstance.idx)
          .style('stroke', 'black')
    ...
    if (this.props.seletedInstanceNNs !== nextProps.selectedInstanceNNs) {
      ...
      d3.selectAll('.rect_output.neighbor')
        .style('stroke', d3.rgb(gs.groupColor1).darker())
        .style('stroke-width', 0.5)
  ...
  render() {
    ...
    const rankingScale = d3
      ...
      selectedNonTopkRankingScale = d3
        .scaleBand()
    ...
    const topkTickValues = [1, ...d3.range(5, topk, 5), topk],
      selectedNonTopkTickValues = [
        topk + 1,
        ...d3.range(topk + 1 + ((topk + 1) % 5), to, 5),
        to
      ];
    ...
    const xTopkAxis = d3
        ...
        .attr(
          'transform',
          'translate(' +
            _self.layout.plot.marginLeft +
            ',' +
            _self.layout.plot.marginTop +
            ')'
        )
        .call(
          d3
            .axisLeft(topkRankingScale)
            .tickValues(topkTickValues)
            .tickSizeOuter(0)
        ),
      xSelectedNonTopkAxis = d3
        .select(svg)
        .append('g')
        ...
        .attr(
          ...
          'translate(' +
            _self.layout.plot.marginLeft +
            ',' +
            (_self.layout.plot.marginTop + topkPlotHeight) +
            ')'
        )
        .call(
          d3
            .axisLeft(selectedNonTopkRankingScale)
            .tickSizeOuter(0)
            .tickValues(d3.range(topk + 1, to, 5))
        );

    const gSelectedIntervalRanking = d3
      .select(svg)
      .append('g')
      .attr('class', 'g_selected_interval_ranking')
      .attr(
        'transform',
        'translate(' + (_self.layout.plot.marginLeft + 10) + ',' + '10)'
      );

    const gSelectedIntervalReranking = d3
      .select(svg)
      .append('g')
      .attr('class', 'g_selected_interval_reranking')
      .attr(
        'transform',
        'translate(' + (_self.layout.plot.marginLeft + 35 + 10) + ',' + '10)'
      );
    ...
    const outputRect = gSelectedIntervalRanking
      .selectAll('.rect_group')
    ...
    const outputRectForPattern = gSelectedIntervalRanking
      .selectAll('.rect_target')
      .data(selectedInstances)
      .enter()
      .append('rect')
