...
class UtilityBar extends Component {
  ...
  render() {
    ...
    const xAxisSetting = d3
        .axisBottom(measureScale)
        .tickValues(measureDomain)
        .tickSize(0),
      xAxis = d3
        .select(svg)
        .append('g')
    ...
    const fairRect = g
      ...
      .attr(
        'transform',
        'translate(' + measureScale(perfectScore) + ',2)' + 'rotate(45)'
      );
    ...
    g.append('path')
      ...
      .attr('transform', function(d) {
        return 'translate(' + measureScale(measure) + ',' + 25 + ')';
      });

    return <div className={styles.measure}>{svg.toReact()}</div>;
  }
}
